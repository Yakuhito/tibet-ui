...
function AddAssetButton({ asset_id, short_name, image_url, name, activeWallet, onCompletion = () => {} }: AddAssetButtonProps) {
    ...
    if (activeWallet instanceof GobyWallet ||  activeWallet instanceof WalletConnect) {
        return ( 
            <button onClick={() => !isLoading && !isAdded ? addAssetToWallet(asset_id, short_name, image_url, name) : null} className={`${isLoading ? 'animate-pulse' : ''} ${isAdded ? 'bg-green-700/20 text-green-700 cursor-default opacity-70' : 'bg-brandDark/10 text-brandDark dark:text-brandLight'} h-fit max-w-[160px] w-[148px] transition font-medium hover:opacity-80 py-1 px-4 whitespace-nowrap rounded-lg flex justify-center items-center gap-3`}>
