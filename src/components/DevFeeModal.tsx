                          ...
                          <RadioGroup className="flex flex-col gap-4" value={selectedOption} onChange={handleOptionChange}>
                            ...
                            {options.map((option) => (
                                <RadioGroup.Option key={option.id} value={option.id} className="outline-none">
                                  {({ active, checked }) => (
                                  <div className={`${checked ? 'border-brandDark' : 'hover:border-brandDark/10'} ${active ? 'ring-4 ring-brandDark/20' : ''} py-8 px-8 bg-brandDark/10 rounded-xl border-2 border-transparent cursor-pointer`}>
                                    {option.label === "Custom 🙌" &&
                                    <>
